...
def create_example_motion_mask(shape: Tuple[int, int], location: str = "center") -> np.ndarray:
    """
    Helper function to create example motion masks for testing.
    ...
    if location == "center":
        cv2.circle(mask, (w // 2, h // 2), 50, 255, -1)
    elif location == "top-left":
        cv2.rectangle(mask, (10, 10), (100, 100), 255, -1)
    elif location == "bottom-right":
        cv2.rectangle(mask, (w - 100, h - 100), (w - 10, h - 10), 255, -1)
    elif location == "unusual":
        # Motion in an unusual corner
        cv2.circle(mask, (w - 50, 50), 30, 255, -1)
...
if __name__ == "__main__":
    # Example usage
    print("Stage 3: Heat Map Analysis Demo")
    print("=" * 50)
    ...
    # Simulate historical motion events (center location - normal pattern)
    print("\n1. Building historical pattern (20 events at center)...")
    for i in range(20):
        motion_mask = create_example_motion_mask((480, 640), "center")
        analyzer.add_motion_event(motion_mask)
    
    # Add some activity at bottom-right (less frequent)
    print("2. Adding some events at bottom-right (5 events)...")
    for i in range(5):
        motion_mask = create_example_motion_mask((480, 640), "bottom-right")
        analyzer.add_motion_event(motion_mask)
    
    # Get statistics
    stats = analyzer.get_statistics()
    print(f"\n3. Heat Map Statistics:")
    for key, value in stats.items():
        print(f"   {key}: {value}")
    
    # Test normal motion (center - should be low anomaly)
    print("\n4. Testing motion at CENTER (expected location):")
    normal_motion = create_example_motion_mask((480, 640), "center")
    scores = analyzer.calculate_anomaly_score(normal_motion)
    print(f"   Anomaly Score: {scores['anomaly_score']:.3f}")
    print(f"   Motion in quiet zones: {scores['motion_in_quiet_zones']:.1f}%")
    print(f"   Motion in active zones: {scores['motion_in_active_zones']:.1f}%")
    print(f"   Is unusual: {analyzer.flag_unusual_motion(normal_motion)}")
    
    # Test unusual motion (top-left - should be high anomaly)
    print("\n5. Testing motion at TOP-LEFT (unusual location):")
    unusual_motion = create_example_motion_mask((480, 640), "top-left")
    scores = analyzer.calculate_anomaly_score(unusual_motion)
    print(f"   Anomaly Score: {scores['anomaly_score']:.3f}")
    print(f"   Motion in quiet zones: {scores['motion_in_quiet_zones']:.1f}%")
    print(f"   Motion in active zones: {scores['motion_in_active_zones']:.1f}%")
    print(f"   Is unusual: {analyzer.flag_unusual_motion(unusual_motion)}")
    
    # Save heat map
    print("\n6. Saving heat map to disk...")
    analyzer.save_heatmap(camera_id="demo_camera")
    print("   Saved successfully!")
    
    print("\n" + "=" * 50)
    print("Stage 3 Demo Complete!")
    print("\nKey Capabilities:")
    print("✓ Maintains rolling history of motion events")
    print("✓ Calculates pixel-level probability heat maps")
    print("✓ Normalizes to 0-1 scale")
    print("✓ Compares current motion against baseline")
    print("✓ Flags motion in unusual locations")
    print("✓ Provides anomaly scores and explanations")
